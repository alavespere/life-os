# user.md — Aaron's Persistent Context
_Last updated: 2026-04-06_

---

## Identity
- **Name:** Aaron
- **Role:** Founder & CEO, BuildSafe IQ Inc.
- **Entity:** Delaware C-Corp (SaaS platform — BuildSafe IQ Inc.) | Wyoming LLC (brokerage — not yet registered, future)
- **Location:** Merrimack, NH
---

## Business Context

**Company:** BuildSafe IQ Inc.
**Mission:** AI-powered construction risk intelligence for commercial GCs,
subcontractors, developers, and investors.

**BuildSafe IQ (Delaware C-Corp) — SaaS Platform:**
- AI-powered construction risk intelligence platform
- Revenue: SaaS subscriptions + risk management services

**Insurance Brokerage (future — separate entity):**
- Wyoming LLC, not yet registered
- P&C, surplus lines — wholesale model
- Intentionally separate from SaaS entity per investor feedback
- Not referenced in current pitch deck

**Go-to-Market:** Wholesale brokers (Halcyon, Burns & Wilcox, AmWINS, RT Specialty).
No direct carrier appointments at launch.

**Tech Stack:** Google Workspace, GCP (Risk Sync), Replit, Anthropic Claude API,
Google Apps Script, Atlassian, Fathom

**Team:**
- Arshdeep Kaur — Pune (contractor, tech) | Active on month-to-month basis
- Ev Jordan — VP of Technology (25 yrs tech, multiple exits)
- Grant Robbins — AE (30+ yrs construction, early Procore validator, co-founded Billy) | No equity yet — future grant contingent on full commitment

**Exited:**
- Deon Gracias — exited due to conflict with full-time job

**Legal Counsel:**
- Morse Barnes-Brown & Pendleton: John Hession, Chris Harris
- Woodward Cass Law: Cole Young, Esq.

---

## Personal Goals (2026)
1. Financial independence / wealth building
2. Family & relationships (meaningful personal connections)
3. Personal growth & learning
4. Health & fitness

---

## Non-Negotiable Daily Habits
1. Morning workout / exercise
2. Cold outreach — minimum 5 touches/day

---

## Current Biggest Bottlenecks
1. Task management (tasks pile up, fall through cracks)
2. Content & marketing output (ideas die in backlog)

---

## Communication Preferences
- Capture method: Email to self → routed into Daily Note Brain Dump
- Personal tracking depth: Moderate (daily habits + monthly reviews)

---

## Content Voice
_(Full guide lives in `2. Content/Voice & Style Guide.md` — read that file for all content generation)_

- **Style model:** Johnny Harris (YouTube) — narrative-first, data-backed, tension-building, one thesis per piece
- **Audience:** Commercial GCs, subcontractors, developers, investors
- **Tone:** Direct, confident, data-grounded, no fluff
- **Reading level:** 8th grade
- **Never:** "I'm excited to share", listicles, "5 things you should know", corporate passive voice
- **Formats:** LinkedIn posts, long-form articles, case studies, video scripts

---

## Active Open Threads
_(Update manually as status changes)_

- **Fundraising:** $2.5M seed at $10M pre-money — active. Pitch deck v.3.30.26 current.
- **Legal (19 open items):** Full tracker in `BuildSafe IQ/Legal/Legal Tracker.md`
  - Critical path: Investment instrument decision → cap table → IP assignment → founder vesting
  - First call: John Hession at Morse Barnes-Brown & Pendleton
- **Brokerage entity:** Separate from SaaS C-Corp. Wyoming LLC not yet registered. NH agency license pending LLC formation.
- **Beta program:** Launching Q2 2026 — first contractor partners being engaged
- **2025 financials:** In progress — accountant handoff needed
- **Trend memory layer:** Next tech build (future)

---

## Changelog

| Date | Change |
|------|--------|
| 2026-04-06 | Initial version created |
